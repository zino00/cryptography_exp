...
class udp_logic(UI.MainUi):
    ...
    def ssl_end_threading(self):
        self.ssl_server_thread.ssl_server_socket.sendall('exit'.encode())
        self.ssl_client_thread.ssl_client_socket.sendall('exit'.encode())
        msg = '客户端线程已关闭'
        self.send_Show_msg(msg)
        self.tab.tab5TextEdit_2.clear()
        msg = '服务端线程已关闭'
        self.send_Show_msg(msg)
        self.tab.tab5TextEdit_1.clear()
        self.ssl_server_thread.ssl_server_socket.close()
        self.ssl_client_thread.ssl_client_socket.close()
        msg = 'socket已关闭'
        self.send_Show_msg(msg)
